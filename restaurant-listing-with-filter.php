...
function rl_register_restaurant_cpt() {
    ...
    $args = array(
        ...
        // <-- on ajoute la prise en charge des catégories natives
        'taxonomies'    => array('category'),
    );
    register_post_type('restaurant', $args);

    // S'assurer que la taxonomie 'category' est bien rattachée
    register_taxonomy_for_object_type('category', 'restaurant');
}
add_action('init', 'rl_register_restaurant_cpt');


// 2. Filtre ACF pour masquer certains champs si la catégorie != 'street-food'
add_filter('acf/prepare_field', 'rl_masquer_champs_selon_categorie');
function rl_masquer_champs_selon_categorie($field) {
    global $post;
    ...
    // si la catégorie 'street-food' n'est pas cochée, on masque ces champs
    if ( ! in_array('street-food', $cats, true) ) {
        $champs_streetfood = ['chariot_mobile', 'point_de_vente', 'prix_max'];
        if ( in_array($field['name'], $champs_streetfood, true) ) {
            return false;
        }
    }
...
// 3. Shortcode pour afficher la liste + filtres
        ...
        while ($query->have_posts()) {
            ...
            echo '<div class="restaurant-card">';
                ...
                echo '<div class="restaurant-right">';
                    ...
                    if ($lien_reservation) {
                        echo '<a class="reserve-button" href="' . esc_url($lien_reservation) . '" target="_blank">Réserver</a>';
                    }
                echo '</div>';
            echo '</div>';
        }
